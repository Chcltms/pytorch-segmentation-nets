...
class PAB(nn.Module):
    def __init__(self):
        ...
        self.score_map = nn.Sequential(
            ...
            nn.Conv2d(42, 42, kernel_size=3, stride=1, padding=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(42, 21, kernel_size=3, stride=1, padding=1, bias=True)
        )
...
class OuputMoudle(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.pab1(out, x[1])
        out = self.transform1(out)
        out = self.pab2(out, x[2])
        out = self.transform2(out)
        out = self.pab3(out, x[3])
...
class ResnetGCN_PAB(nn.Module):
    def __init__(self, class_num=6, resnet_depth=18, kernal_size=15, feature_num=21, init_resnet_state_path=None,
        ...
        super(ResnetGCN_PAB, self).__init__()
